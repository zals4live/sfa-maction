...
// Admin portal entry point — deliberately no auth middleware.
definePageMeta({
  layout: 'auth'
})
...
/** Authenticate via the store, then route to the admin dashboard on success. */
...
onMounted(async () => {
  if (authStore.isAuthenticated) await navigateTo('/admin/dashboard')
})
</script>
  ...
  <div class="flex flex-col gap-6">
    <div class="flex flex-col gap-1 text-center">
      ...
      <p class="text-sm text-muted">
        Masuk ke portal administrasi KF Maction
      </p>
    </div>
